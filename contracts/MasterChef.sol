...
contract SurfToken is ERC20("SurfToken", "SURF"), Ownable {
    uint256 public constant MAX_SUPPLY = 250_000_000e18; // 250 million surf

    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(
            totalSupply() + _amount <= MAX_SUPPLY,
            "SURF::mint: cannot exceed max supply"
        );
        _mint(_to, _amount);
    }
}
...
/*
    This master chef is based on SUSHI's version with some adjustments:
     ...
     - update of emission rate with upper limit of 6 SURF/block
     - more require checks in general
*/
contract SurfMasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of SURF
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accSurfPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accSurfPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Info of each pool.
    struct PoolInfo {
        // we have a fixed number of SURF tokens released per block, each pool gets his fraction based on the allocPoint
        uint256 allocPoint; // How many allocation points assigned to this pool. the fraction SURF to distribute per block.
        uint256 lastRewardBlock; // Last block number that SURF distribution occurs.
        uint256 accSurfPerShare; // Accumulated SURF per LP share. this is multiplied by ACC_SURF_PRECISION for more exact results (rounding errors)
    }
    // The SURF TOKEN!
    SurfToken public surf;

    // Treasury address.
    address public treasuryAddress;

    // SURF tokens created per block.
    uint256 public surfPerBlock;

    uint256 private constant ACC_SURF_PRECISION = 1e12;
    ...
    event UpdateEmissionRate(address indexed user, uint256 _surfPerSec);

    constructor(
        SurfToken _surf,
        address _treasuryAddress,
        uint256 _surfPerBlock,
        uint256 _startBlock
    ) {
        require(
            _surfPerBlock <= 6e18,
            "maximum emission rate of 6 surf per block exceeded"
        );
        surf = _surf;
        treasuryAddress = _treasuryAddress;
        surfPerBlock = _surfPerBlock;
    ...
    // Update the given pool's SURF allocation point. Can only be called by the owner.
    ...
    // View function to see pending SURF on frontend.
    function pendingSurf(uint256 _pid, address _user)
        external
        view
        returns (uint256 pending)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            // based on the pool weight (allocation points) we calculate the surf rewarded for this specific pool
            uint256 surfRewards = (blocksSinceLastReward *
                surfPerBlock *
                pool.allocPoint) / totalAllocPoint;

            // we take parts of the rewards for treasury, these can be subject to change, so we recalculate it
            // a value of 1000 = 100%
            uint256 surfRewardsForPool = (surfRewards * POOL_PERCENTAGE) /
                1000;

            // we calculate the new amount of accumulated surf per LP token
            accSurfPerShare =
                accSurfPerShare +
                ((surfRewardsForPool * ACC_SURF_PRECISION) / lpSupply);
        }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                ...
                surf.mint(
                    treasuryAddress,
                    (surfRewards * TREASURY_PERCENTAGE) / 1000
                );

                surf.mint(address(this), surfRewardsForPool);

                pool.accSurfPerShare =
                    pool.accSurfPerShare +
                    ((surfRewardsForPool * ACC_SURF_PRECISION) / lpSupply);
            }
    ...
    // Deposit LP tokens to MasterChef for SURF allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _to
        ...
        // note that only the accSurfPerShare have the precision applied
        user.rewardDebt =
            user.rewardDebt +
            (_amount * pool.accSurfPerShare) /
            ACC_SURF_PRECISION;

        IRewarder _rewarder = rewarder[_pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onSurfReward(_pid, _to, _to, 0, user.amount);
        }
    ...
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _to Receiver of SURF rewards.
    function harvest(uint256 _pid, address _to) public {
        ...
        uint256 accumulatedSurf = (user.amount * pool.accSurfPerShare) /
            ACC_SURF_PRECISION;
        // subtracting the rewards the user is not eligible for
        uint256 eligibleSurf = accumulatedSurf - user.rewardDebt;

        // we set the new rewardDebt to the current accumulated amount of rewards for his amount of LP token
        user.rewardDebt = accumulatedSurf;

        if (eligibleSurf > 0) {
            safeSurfTransfer(_to, eligibleSurf);
        }

        IRewarder _rewarder = rewarder[_pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onSurfReward(
                _pid,
                msg.sender,
                _to,
                eligibleSurf,
        ...
        emit Harvest(msg.sender, _pid, eligibleSurf);
    }

    /// @notice Withdraw LP tokens from MCV and harvest proceeds for transaction sender to `_to`.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _amount LP token amount to withdraw.
    /// @param _to Receiver of the LP tokens and SURF rewards.
    function withdrawAndHarvest(
        ...
        /*
            ...
            => userInfo.amount * pool.accSurfPerShare / ACC_SURF_PRECISION == userInfo.rewardDebt
    ...
    function emergencyWithdraw(uint256 _pid, address _to) public {
        ...
        IRewarder _rewarder = rewarder[_pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onSurfReward(_pid, msg.sender, _to, 0, 0);
        }
    ...
    // Safe SURF transfer function, just in case if rounding error causes pool to not have enough SURF.
    function safeSurfTransfer(address _to, uint256 _amount) internal {
        uint256 surfBalance = surf.balanceOf(address(this));
        if (_amount > surfBalance) {
            surf.transfer(_to, surfBalance);
        } else {
    ...
    function updateEmissionRate(uint256 _surfPerBlock) public onlyOwner {
        require(
            _surfPerBlock <= 6e18,
            "maximum emission rate of 6 surf per block exceeded"
        );
        surfPerBlock = _surfPerBlock;
        emit UpdateEmissionRate(msg.sender, _surfPerBlock);
    }
}